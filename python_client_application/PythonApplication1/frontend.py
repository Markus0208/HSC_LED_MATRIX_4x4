...
class ImageProcessingApp:
    ...
    def send_image(self):
        ...
        if self.cyclic_enabled and not self.is_sending_cyclic:  # Zyklus hat Priorität
            self.start_cyclic_send()
        elif self.selected_images:
            for widget_id in self.selected_images:  # Durch Widget-IDs iterieren
                widget = self.image_frame.nametowidget(widget_id)
                self.send_single_image(widget.image_path)  # Pfad aus Widget extrahieren
